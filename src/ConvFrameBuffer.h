...
#pragma --mark "卷积FrameBuffer Cache"
...
//    }
    ...
    uint32_t vertexCount(){
        return (uint32_t)m_vertices.size()/2;
    }
    uint32_t indexCount(){
        return (uint32_t)m_indexs.size();
    }
    // 将一个通道分成几个三角形，返回数量为顶点数，为三角形数+1
    static uint32_t convChannelRect();
...
#pragma --mark "卷积Framebuffer Cache"
class ConvBufferCache: public GPUBufferCache{
public:
    static ConvFrameBuffer* getFrameBuffer(int channel_width, int channel_height, int padding_size, int x_count, int y_count, int count=0);
};
#endif
