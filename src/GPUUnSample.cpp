...
void GPUUnSample::activeOutFrameBuffer(){
    ...
    if (m_outbuffer == NULL) {
        m_outbuffer = ConvBufferCache::getFrameBuffer(m_unsample_size*buffer->m_channel_width, m_unsample_size*buffer->m_channel_height, buffer->m_padding_size, buffer->m_x_count, buffer->m_y_count);
