...
void GPUAdaIn::activeOutFrameBuffer(){
    ConvFrameBuffer* buffer = dynamic_cast<ConvFrameBuffer*>(m_input_buffers[2]);
    if (m_outbuffer == NULL) {
        m_outbuffer = ConvBufferCache::getFrameBuffer(buffer->m_channel_width, buffer->m_channel_height, buffer->m_padding_size, buffer->m_x_count, buffer->m_y_count);
    ...
    setInputFrameBuffer(m_var_buffer, 3);
    setInputFrameBuffer(m_mean_buffer, 4);
