...
void GPUPadding::render(){
    ...
    GPUCheckGlError(m_filter_name.c_str(), true, false);
    glDrawElements(GL_TRIANGLES, dynamic_cast<ConvFrameBuffer*>(m_outbuffer)->vertexCount(), GL_UNSIGNED_SHORT, ((ConvFrameBuffer*)m_outbuffer)->vertexIndexs());
    glFlush();
    GPUCheckGlError(m_filter_name.c_str(), true, false);
...
void GPUPadding::activeOutFrameBuffer(){
    if (m_outbuffer == NULL) {
        ...
        m_outbuffer = new ConvFrameBuffer(input_frame->m_channel_width, input_frame->m_channel_height,
                                          m_padding_size, m_outx_count, m_outy_count);
